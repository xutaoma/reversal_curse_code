...
def preprocess_packed_supervised_dataset(
    ...
    valid_num = 0
    batch_input_ids, batch_labels, batch_images, batch_videos, batch_weights = [], [], [], [], []
    lengths = []
    ...
    for i in range(len(examples["_prompt"])):
        ...
        if length > data_args.cutoff_len:
            ...
            batch_weights.append(examples["_weight"][i])
            valid_num += 1
    ...
    for knapsack in knapsacks:
        ...
        # For packed sequences, use default weight 1.0 (per-sample weights are not
        # well-defined once multiple samples are packed into one sequence).
