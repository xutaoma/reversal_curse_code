...
def convert_alpaca(
    ...
    wt = 1.0
    if dataset_attr.weight and dataset_attr.weight in example:
        try:
            wt = float(example[dataset_attr.weight])
            if wt < 0:
                logger.warning_rank0(f"Invalid weight {wt}, using 1.0")
                wt = 1.0
        except (ValueError, TypeError):
            logger.warning_rank0(f"Invalid weight value, using 1.0")
            wt = 1.0
