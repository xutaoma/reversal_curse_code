...
def get_custom_gradient_checkpointing_func(gradient_checkpointing_func: Callable) -> Callable:
    ...
    def custom_gradient_checkpointing_func(func: Callable, *args: Union["torch.Tensor", Any], **kwargs):
        # Handle both bound methods and functools.partial objects (for newer transformers versions)
        from functools import partial

        if isinstance(func, partial):
            # For partial objects, try to get the module from the wrapped function
            module = getattr(func.func, "__self__", None)
        else:
            # For bound methods, get __self__ directly
